...
#ifndef BRISCOLA_MCTS_H
#define BRISCOLA_MCTS_H
...
protected:
     ...
     */
    virtual void print(std::ostream &strm) const { strm << this; };
...
template<class T>
class Action {
    friend std::ostream &operator<<(std::ostream &strm, const Action<T> &a) {
        a.print(strm);
        return strm;
    };
...
class MCTS {
    ...
    std::shared_ptr<Backpropagation<T>> backprop;
    std::shared_ptr<TerminationCheck<T>> termination;
    std::shared_ptr<Scoring<T>> scoring;
...
public:
     ...
     */
    Node<T, A, E> * getRoot() { return root.get(); }

private:
    void search() {
        std::chrono::system_clock::time_point old = std::chrono::system_clock::now();

        while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - old) <
               allowedComputationTime || iterations < minIterations) {
            iterations++;
    ...
    std::shared_ptr<Node<T, A, E>> select(const Node<T, A, E> &node) {
        ...
        if (node.getNumVisits() < minVisits) {
            std::uniform_int_distribution<unsigned int> distribution(0, children.size() - 1);
